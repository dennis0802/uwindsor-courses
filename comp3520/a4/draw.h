...
void drawTexturedPlane(float* a, float* b, float* c, float* d, unsigned char* data, GLsizei width, GLsizei height);
...
// Performs the same as drawPlane, but works with texture functions
void drawTexturedPlane(float* a, float* b, float* c, float* d, unsigned char *data, GLsizei width, GLsizei height) {
    float n[3];
    crossProduct(a, b, c, n);
    glNormal3fv(n);
    ...
    // Texture pattern to enlarge to fit target coordinate range
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

    // Texture pattern to minify to fit target coordinate range
...
void drawOffice(float roomLength) {
    ...
    c[0] = roomLength;  c[1] = -10;       c[2] = -roomLength;
    d[0] = -roomLength; d[1] = -10;       d[2] = -roomLength;
    drawTexturedPlane(a, b, c, d, carpetData, carpetWidth, carpetHeight);
}
...
void drawTable(void) {
    ...
    drawTexturedPlane(a, b, c, d, woodData, woodWidth, woodHeight);
